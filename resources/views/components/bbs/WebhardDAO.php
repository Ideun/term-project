...
class WebhardDao {
    ...
    public function __construct() {
        try {
            $this->db = new PDO("mysql:host=localhost;dbname=php",
                                "root", "");
    ...
    public function getFileList($sort, $dir) {
        try {
            $query = $this->db->prepare("select * from webhard
                                         order by $sort $dir");
            $query->execute();
    ...
    public function deleteFileInfo($num) {
        try {
            $query = $this->db->prepare("select fname from webhard
                                         where num=:num");
            ...
            $query = $this->db->prepare("delete from webhard
                                         where num=:num");
