...
class BoardDAO {
    ...
    /* 댓글 */
      public function insertCmt($bid,$cwriter, $comment) {

        try {
            $sql = "insert into comment(bid, cwriter, comment) values(:bid, :cwriter, :comment)";
            $pstmt = $this->db->prepare($sql);
            $pstmt->bindValue(":bid", $bid, PDO::PARAM_INT);
    ...
    public function getCmt($bid) {
        try {
            $sql = "select * from comment where bid=:bid";
            $pstmt = $this->db->prepare($sql);
            $pstmt->bindValue(":bid", $bid, PDO::PARAM_STR);
            $pstmt->execute();
    ...
    public function updateMsg($num, $title, $writer, $content){
        try{
            ...
            */
            $sql = "update board set title=:title, writer=:writer,
